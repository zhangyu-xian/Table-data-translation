...
promptContentScoreText = """
你的任务是评估两个xml的内容相似程度。使用1到10之间的数据作为指标，可以是小数。
用户以字符串的格式将提供两个xml，您必须根据下方提供的评估标准，从1到10之间选择一个指标。
请务必仔细阅读并理解这些说明，最终的输出格式为{"内容相似性":""}
请在评估期间保持此文档处于打开状态，并在需要时参考。

评估步骤：
1. 读取和理解两个xml。读取获得的两个xml，确保充分理解两个xml内容。
2. 对比两个xml包含的元素。比较元素的标签、内容是否一致。
3. 对比两个xml的属性。比较属性和属性值是否一致。
4. 对比两个xml的文本。比较文本内容是否一致。

Question: 请你评估两个xml的内容相似程度:
{xml_ground_truth}和{xml_predicated}
Answer:
"""


promptStructureScoreText = """
你的任务是评估两个xml的结构相似程度。使用1到10之间的数作为指标，可以是小数。
用户以字符串的格式将提供两个xml，您必须根据下方提供的评估标准，从1到10之间选择一个数作为指标。
请务必仔细阅读并理解这些说明，最终的输出格式为{"结构相似性":""}
请在评估期间保持此文档处于打开状态，并在需要时参考。

评估步骤：
1. 读取和理解两个xml。读取获得的两个xml，确保充分理解两个xml的结构。
2. 对比两个xml包含的元素数量是否一样。
3. 对比两个xml包含的具有属性的元素的数量是否一样。

Question: 请你评估两个xml的结构相似程度:
{xml_ground_truth}和{xml_predicated}
Answer:
"""
